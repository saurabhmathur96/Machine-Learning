    ...
    def _predict(self, inputs, node):
        if type(node) is LeafNode:
            ...
            if value in node.children:
                ...
                return node.majority_target_value
...
def _create_tree(rows, target, attributes):
    ...
    majority_target_value = Counter(row[target] for row in rows).most_common(1)[0][0]

    children = {value: (_create_tree(split_rows, target, remaining_attributes) if entropy(split_rows, target) > 0 else LeafNode(value, split_rows[0][target])) 
                for (value, split_rows) in splits[splitting_attribute].iteritems()}

    return DecisionNode(splitting_attribute, majority_target_value, children)
